...
(function () {
  ...
  window.checkQuiz = function (containerId) {
    ...
    $$('.quiz-question-block', container).forEach(q => {
      ...
      $$('.quiz-option', q).forEach(o => { o.disabled = true; o.setAttribute('aria-disabled', 'true'); });
  ...
  /* ── ARCH DIAGRAM ENGINE ───────────────────────────────────── */
  window.showArchDesc = function (el, description) {
    const descEl = $('.arch-description');
    if (descEl) {
  ...
  // Keyboard support for arch-component buttons
  document.addEventListener('keydown', function (e) {
    if (e.key !== 'Enter' && e.key !== ' ') return;
    const arch = e.target.closest('.arch-component');
    if (!arch) return;
    e.preventDefault();
    arch.click();
  });
  ...
  function initChat(containerEl) {
    ...
    document.addEventListener('click', function (e) {
      ...
      if (action === 'restart') restartChat(btn);
  ...
  function initFlow(containerEl) {
    ...
    if (labelEl) { labelEl.setAttribute('role', 'status'); labelEl.setAttribute('aria-live', 'polite'); }
